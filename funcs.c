...
long solveMaxFlowAccVER4(cType root, long minCandi, NodePropArr* pnp, cType s, cType t, int SPAN_LEN, aType LN)
{
  ...
  // printf("1mcv: %ld\n",mcv);
    while (1 == 1)
    {
      ...
      {
        //上个段尾还在t下面，t和段尾平齐也没关系，此时整段可以直接跳过
      ...
      //注意，一个段的前LN-1个节点,作为祖先节点其upid_LC是没有的
      //所以，如果LN_s是一个段的前LN-1个节点，其paccup_LC是没法用的，需要s自己提升过去，这种情况也不多见
      if (paccup_LC[LN_s] == 0)
      {
      ...
      {
        s = paccup_LC[LN_s];
        // printf("check: (LN_s is %ld, dep is %ld ) s is %ld, dep is %ld  \n",LN_s,pDep[LN_s], s, pDep[s]);
  ...
  //现在都是正常的LN_s 和 LN_t了，可以快乐的进行整段回溯了
  // printf("4mcv: %ld\n",mcv);
  ...
  if(t == LN_t || s == LN_s){
    //说明有一个就是另一个祖先，此时，求祖先以上是否包含最优没有意义，是否包含都是一样的，要么都去掉，要么都不去掉
    goto end2;
  }

  cType V = LN_s;
  //向上直到LN祖先，因为LN再往上，合并点这边就是确定的了
  //确切的说，需要计算s到LN_s(不含),t到LN_t(不含)对给定祖先点V的情况，如果1/0或0/1都可以，而且可以计算最优减少值
  //

  //注意：这里不用保证V是s的LN祖先以内，其实只要是LN_s的祖先以内也可以，因为此时涉及跨2个LN段的逻辑
  for (; pDep[LN_s] - pDep[V] < LN; V = pFa[V])
  {
    ...
    if(inc_s == 2 || inc_t == 2){
      //最优在LN_s(LN_t)之上断开了，没法参与计算了
      continue;
    }

    //下面如果有不包含，都是在v_top(不含)以下的
    if (inc_s + inc_t == 2)
    {
      //最优都排除不包含，略过继续向上
      continue;
